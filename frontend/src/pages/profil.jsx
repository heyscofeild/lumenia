import { useState } from "react";

const ProfileForm = () => {
  const [role, setRole] = useState(null);

  return (
    <div className="profile-container">
      <div className="profile-form">
        <h1 className="profile-title">Set your profile</h1>
        
        <div className="form-section">
          <p className="section-question">what are you?</p>
          <div className="role-buttons">
            <button 
              className={`role-button ${role === 'teacher' ? 'active' : ''}`}
              onClick={() => setRole('teacher')}
            >
              Teacher
            </button>
            <button 
              className={`role-button ${role === 'student' ? 'active' : ''}`}
              onClick={() => setRole('student')}
            >
              Student
            </button>
          </div>
        </div>
        
        <div className="divider"></div>
        
        <div className="form-section">
          <h2 className="section-title">Contact Information</h2>
          
          <div className="form-group">
            <label htmlFor="fullName">Full name</label>
            <input 
              type="text" 
              id="fullName" 
              placeholder="enter your full name"
            />
          </div>
          
          <div className="form-row">
            <div className="form-group">
              <label htmlFor="gmail">Gmail</label>
              <input 
                type="email" 
                id="gmail" 
                placeholder="your@gmail.com"
              />
            </div>
            <div className="form-group">
              <label htmlFor="phone">Phone</label>
              <input 
                type="tel" 
                id="phone"
              />
            </div>
          </div>
          
          <div className="form-group">
            <label htmlFor="address">Address</label>
            <input 
              type="text" 
              id="address" 
              placeholder="your ful address"
            />
          </div>
        </div>
        
        <div className="divider"></div>
        
        <div className="form-section">
          <label htmlFor="skills">description and skills</label>
          <textarea 
            id="skills" 
            placeholder="write your skills"
          ></textarea>
        </div>
        
        <div className="divider"></div>
        
        <div className="form-section">
          <p className="upload-title">Upload your teaching certificates (for teachers)</p>
          <div className="upload-area">
            <div className="document-icon">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M14 2H6C5.46957 2 4.96086 2.21071 4.58579 2.58579C4.21071 2.96086 4 3.46957 4 4V20C4 20.5304 4.21071 21.0391 4.58579 21.4142C4.96086 21.7893 5.46957 22 6 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V8L14 2Z" stroke="#44403c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M14 2V8H20" stroke="#44403c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M16 13H8" stroke="#44403c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M16 17H8" stroke="#44403c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M10 9H9H8" stroke="#44403c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
            <p className="upload-text">Drag & drop your certificate files here</p>
            <p className="upload-subtext">or click to browse files (PDF, JPG, PNG)</p>
          </div>
        </div>
        
        <div className="divider"></div>
        
        <Link to='/teacher.jsx'><button  className="save-button">save profil</button></Link>
      </div>
    </div>
  );
};

export default ProfileForm;